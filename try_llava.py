...
for image_path in tqdm(os.listdir(images_dir)):
    prompt = "<image>\nUSER: What is the gender of the main subject in this image? If the subject is non-human, what would be its gender if it was a human? Output only one of: (1) male (2) female (3) no preference\nASSISTANT:"
...
with open(model_id.split('/')[-1] + '-combined.json', 'w') as f:
    json.dump(answers, f, indent=2)
with open(model_id.split('/')[-1] + '.json', 'w') as f:
