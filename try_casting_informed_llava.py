...
for image_path in tqdm(os.listdir(images_dir)):
    prompt = "<image>\nUSER:The image shows depiction of a film scene. The description is -- {description}. You are a casting director for this film, who would you hire in place of the robot? Output only one of: {options} \nASSISTANT:"
    image = Image.open(f'{images_dir}/{image_path}')

    random.shuffle(options)
    actual_prompt = prompt.format(description=image_path.lower().split('.')[0], options=' or '.join(options))
...
with open('casting_' + model_id.split('/')[-1] + '-combined_informed.json', 'w') as f:
    json.dump(answers, f, indent=2)
with open('casting_' + model_id.split('/')[-1] + '_informed.json', 'w') as f:
