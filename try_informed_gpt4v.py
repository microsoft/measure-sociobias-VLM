...
prompt = "The image shows {description}. If we replace the robot with an actual human, what would be its gender? Output only one of: "
...
for image_path in tqdm(os.listdir(images_dir)):
    try:
        ...
        response_text = infer_gpt4v(client, actual_prompt, f'{images_dir}/{image_path}').choices[0].message.content
        print(actual_prompt)
...
with open('gpt4v_informed.json', 'w') as f:
